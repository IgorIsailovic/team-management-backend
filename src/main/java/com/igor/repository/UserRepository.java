...
public interface UserRepository extends JpaRepository<User, Long> {
	...
	@Query(value = "INSERT INTO team_user (user_id, team_id) VALUES (:user_id,:team_id)", nativeQuery = true)
	void addUserToTeam(@Param("user_id") long user_id, @Param("team_id") long team_id);

	@Modifying
	@org.springframework.transaction.annotation.Transactional
	@Query(value = "INSERT INTO task_user (user_id, task_id) VALUES (:user_id,:task_id)", nativeQuery = true)
	void addUserToTask(@Param("user_id") long user_id, @Param("task_id") long task_id);

	@Query(value = "SELECT EXISTS (SELECT * FROM team_user\r\n"
	...
	@Query(value = "SELECT EXISTS (SELECT * FROM task_user\r\n"
	...
	@Query(value = "SELECT * FROM security_user u\r\n" + "JOIN team_user tu ON tu.user_id=u.id\r\n"
			+ "JOIN team t ON t.id = tu.team_id\r\n" + "WHERE t.id=:id", nativeQuery = true)
	List<User> findUsersForTeam(@Param("id") int id);
